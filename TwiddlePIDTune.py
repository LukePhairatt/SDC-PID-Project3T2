...
class robot:
    ...
    def move(self, steering, distance, 
             tolerance = 0.001, max_steering_angle = 1.0):
...
def run(params, printflag = False):
    ...
    myrobot.set_noise(0.01,0.05)
    speed = 30*0.4477                 # mph to m/s
    dt = 0.15
    dist = speed * dt
    N = 200    
    err = 0.0    
    ...
    myrobot.set_steering_drift(0.5 / 180.0 * pi) # 10 degree steering error
    ...
    for i in range(N * 2):
        ...
        myrobot = myrobot.move(steer, dist)
        # this allows the PID turning to reach the minimal error state
        # sum of err*err
        if i >= N:
...
def twiddle(tol = 0.00005): #Make this tolerance bigger if you are timing out!
    # -------------
    # Add code here
    # -------------
    # p = kp,kd,ki 
    ...
    # uncomment this if tuning only kp by setting kd,ki = 0
    #dp[1] = 0.
    #dp[2] = 0.
